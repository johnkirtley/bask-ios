...
A modern onboarding flow that feels like a **"Personal Consultation"**, not a form. Uses AI-powered skin analysis and HealthKit integration to minimize manual input.

### Strategy: Maximize Conversion
- **Objective-first**: Start with "Why are you here?" to personalize the experience
- **AI-powered**: Use Vision/CoreML for automatic skin type detection (no manual color picking)
...
- [ ] **AI-powered skin scan (new Screen) (MOAT FEATURE)**
  Replace manual color swatches with Vision + CoreML analysis:
  - Show circular frame with front camera preview
  - "Align your face in the circle to determine your UV sensitivity"
  - Use Vision framework to detect face, CoreML to analyze skin tone
  - Map to Fitzpatrick Type I-VI automatically
  - On-device processing (privacy: no photos stored or uploaded)
  - Fallback: If analysis fails, show manual swatch picker
...
- [ ] **VisionKit DataScanner for supplement bottles (MOAT FEATURE)**
  Zero-typing supplement logging using iOS 16+ DataScanner API:
  - Tap "Scan Supplement Bottle" button
  - Point camera at supplement label
  - VisionKit automatically detects and extracts "5000 IU" or "125 mcg" text
  - Pre-fills dosage in supplement log form
  - "Point and Click" logging experience (no manual typing)
...
> 4. **Same pattern for all native integrations**: WeatherKit, HealthKit, Vision/CoreML, and VisionKit all follow this identical bridge pattern
>
...
- [ ] **Custom Capacitor HealthKit Plugin (native Swift side)**
  ...
  - Extend `CAPPlugin` with `@objc` methods
  - Expose `requestAuthorization(call: CAPPluginCall)` -- requests read/write access for `timeInDaylight`, `dietaryVitaminD`, and `stateOfMind`
  - Expose `getTimeInDaylight(call: CAPPluginCall)` -- accepts `startDate`, `endDate`, returns total minutes outdoors from `HKQuantityType(.timeInDaylight)` (iOS 17+)
  - Expose `getDietaryVitaminD(call: CAPPluginCall)` -- reads `HKQuantityType(.dietaryVitaminD)` samples for a date range
  - Expose `writeDietaryVitaminD(call: CAPPluginCall)` -- writes supplement intake to HealthKit
...
#### Vision + CoreML (Skin Analysis)
- [ ] **Vision framework integration**
  iOS 13+ face detection for onboarding skin scan. Use `VNDetectFaceRectanglesRequest` to ensure user's face is properly framed

- [ ] **CoreML skin tone model**
  Train or use pre-trained model to analyze skin tone from front camera image. Map RGB values to Fitzpatrick Type I-VI. Process on-device (no server upload)

- [ ] **On-device processing (privacy)**
  All Vision/CoreML analysis runs locally on iPhone. No photos stored or transmitted. Complies with Apple privacy guidelines

#### VisionKit (Supplement Scanning)
- [ ] **DataScanner API integration**
  iOS 16+ VisionKit `DataScannerViewController` for text recognition. Point at supplement bottle label, extract "5000 IU" or "125 mcg" text

- [ ] **Dosage parsing logic**
  Convert scanned text to standardized IU format. Handle "mcg" to IU conversion (1 mcg = 40 IU). Handle formats like "5000IU", "5,000 IU", "125mcg"
...
Apple review requirements for HealthKit, WeatherKit, location, and camera/Vision APIs.

### Implementation Status

#### On-Device Processing (Privacy by Design)
- [ ] **Vision/CoreML on-device analysis**
  All skin scan analysis (onboarding) runs locally using CoreML. No photos stored to disk or uploaded to server. Complies with Apple privacy rules for camera/biometric data

- [ ] **No server-side photo processing**
  Document in privacy policy: "Skin analysis happens entirely on your device. Photos are processed in memory and immediately discarded. We never see or store your images."
...
- [ ] **Transparency during permissions**
  ...
  - **Camera**: "Used once during setup to analyze skin tone. Photo is processed on-device and immediately deleted."
...
- [ ] **Privacy Policy URL**
  ...
  - Skin analysis (camera) happens on-device; no photos stored
...
6. History page (sessions + supplements list) — **Section 6**
7. Calendar view + streaks — **Section 6**
8. Daily decay model + visualization — **Section 1, 3**

### Phase 3: Strategic Moat Features
9. **D-Window Forecast** (48h engine + dashboard card + 48h API data) — **Section 1, 3, 8** (MOAT)
10. **Weather-Adjusted Supplement Advice** (engine + UI) — **Section 1, 4** (MOAT)
11. **Blood Test Integration** (input UI + recalibration) — **Section 12** (MOAT)
12. **SAD Protocol** (Mood vs D graph + HealthKit State of Mind) — **Section 7, 8** (MOAT)

### Phase 4: Education & Insights
13. Insights page content (D/K2/Mg education) — **Section 7**
14. Personalized recommendations — **Section 7**

### Phase 5: Native Integration
15. HealthKit integration (dietaryVitaminD, timeInDaylight, State of Mind) — **Section 8**
16. Notification system (solar noon, decay nudges, cofactor reminders) — **Section 9**
17. HealthKit "pudge" notifications — **Section 9**

### Phase 6: Monetization & Compliance
18. Lab test partner affiliate integration — **Section 12** (MOAT revenue)
19. App Store compliance (attribution, privacy policy, screenshots) — **Section 10**
20. Cleanup (rename DB, remove template CSS, fix links) — **Section 13**
21. Code deduplication (onboarding logic) — **Section 13**
...
- **Native-First Plugin Pattern**: All Apple framework integrations (WeatherKit, HealthKit, Vision, VisionKit) use the same architecture: **Native Swift class → Custom Capacitor Plugin → TypeScript interface → React hook**. See Section 8 "Architecture: Native-First Plugin Pattern" for full details
